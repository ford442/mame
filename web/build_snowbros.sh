...
ROM_PATH="${MAME_DIR}/roms/snowbros.zip"
CORES=55  # Adjust based on available CPU cores

# Common flags shared between both steps
COMMON_FLAGS=(
    "SUBTARGET=snowbros_web"
    "OSD=sdl"
    "TOOLS=0"
    "USE_QTDEBUG=0"
    "NOWERROR=1"
    "USE_BGFX=0"
    "SOURCES=src/mame/kaneko/snowbros.cpp"
)
...
WEB_LDFLAGS="-lGL -sASYNCIFY -sALLOW_MEMORY_GROWTH=1 -sDISABLE_EXCEPTION_CATCHING=0 -sLEGACY_GL_EMULATION=1 -sFORCE_FILESYSTEM=1 -sUSE_SDL=2 -sASSERTIONS=0 --embed-file ${ROM_PATH}@/roms/snowbros.zip"
